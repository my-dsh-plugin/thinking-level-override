...
export const zh: { [Key in Dictionary]: string } = {
  nav: '思考等级',
  title: '思考等级',
  intro: '勾选每个模型可提供的思考等级——对话的模型选择器只会显示这些选项，具体选哪个等级也在那里选。打开「模型映射」可编辑各等级实际发送的值（off 留空 = 发送空值）。',
  ...
  mappingsUnavailable: '设置服务未暴露该插件的命名空间，模型映射不可用；等级勾选不受影响。',
  modelLevel: '思考级别',
  selectLevels: '选择等级',
  levelsSelected: '已选择',
  offNote: '关闭思考',
...
export const en = {
  ...
  intro: 'Check the thinking levels each model offers — the model picker in a conversation shows exactly these, and choosing one happens there. With Model mappings on, edit the value actually sent for each level (blank off = send nothing).',
  ...
  mappingsToggle: 'Model mappings',
  mappingsHint: 'While off, only the spelling editor is hidden; saved levels and spellings stay in effect.',
  mappingsUnavailable: 'The settings service does not expose this plugin\'s namespace, so model mappings are unavailable; level selection still works.',
